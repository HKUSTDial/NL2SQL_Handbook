...
As mentioned before, the \nlsql task is challenging due to the complexity and ambiguity of \nlq queries, as well as the formal and structured nature of \sql. Thus, researchers try to simplify this process by designing a *grammar-free* intermediate representation compared to \sql as the bridge between the ''free-form'' \nlq query and the ''constrained and formal'' \sql query.Roughly speaking, an intermediate representation (IR) is a structured yet flexible grammar that captures the essential components and relationships of an \nlq query without the strict syntax rules of \sql. 
